...
legend('subepi','midwall','subendo','y=x');


figure; hold on;
p(1)=scatter(Control.A11(1:end),C_C11(1:end-1),'filled','r');
LineEpi = polyfit(Control.A11(1:end),log(C_C11(1:end-1)),1);
Ath = 0:0.1:1;
LogCth = LineEpi(1)*Ath + LineEpi(2);
Cth = exp(LogCth);
plot(Ath,Cth,'r');
p(2)=scatter(Control.A12(1:end),C_C12(1:end-1),'filled','g');
LineMid = polyfit(Control.A12(1:end),log(C_C12(1:end-1)),1);
Ath = 0:0.1:1;
LogCth = LineMid(1)*Ath + LineMid(2);
Cth = exp(LogCth);
plot(Ath,Cth,'g');
p(3)=scatter(Control.A13(1:end),C_C13(1:end-1),'filled','b');
LineEndo = polyfit(Control.A13(1:end),log(C_C13(1:end-1)),1);
Ath = 0:0.1:1;
