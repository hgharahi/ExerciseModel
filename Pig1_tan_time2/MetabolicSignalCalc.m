function test = MetabolicSignalCalc(test, So, MetabolicDrive)
...
for j = 1:length(CPP)
    ...
    test.Mtotal(j) = Q_total(j)*60/test.LVweight * (test.ArtO2Cnt(j) - test.CVO2Cnt(j))*10;
%         Mtotal(j) = Q_total(j) *  (Sa - Sv); 
    ...
    test.endo.MVO2(j) = test.Mtotal(j)*Mendo;
    test.mid.MVO2(j) = test.Mtotal(j)*Mmid;
    test.epi.MVO2(j) = test.Mtotal(j)*Mepi;
    ...
    switch MetabolicDrive
        case 'QM'
            if isfield(test,'Exercise_LvL')==0
                ...
                test.endo.MetSignal(j) = test.Exercise_LvL*test.endo.MVO2(j)*q_endo;
                test.mid.MetSignal(j) = test.Exercise_LvL*test.mid.MVO2(j)*q_mid;
                test.epi.MetSignal(j) = test.Exercise_LvL*test.epi.MVO2(j)*q_epi;
            end
        case 'ATP'
