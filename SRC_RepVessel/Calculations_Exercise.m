...
Case.endo.RA = sum(((Ppa - Pendo)./Qendo).*Dt(t_idx(2:end)))/(8*T);
Case.endo.Pl = (Ppa + Pendo)/2;
Case.endo.Ptm = sum((Case.endo.Pl -   Case.endo.Pim).*Dt(t_idx(2:end)))/(8*T);
Case.endo.PC =   sum((Pendo -   Case.endo.Pim).*Dt(t_idx(2:end)))/(8*T);
Case.endo.Q = sum((Qendo).*Dt(t_idx(2:end)))/(8*T);
Case.endo.Pc = sum((Pendo).*Dt(t_idx(2:end)))/(8*T);
...
Case.mid.RA = sum(((Ppa - Pmid)./Qmid).*Dt(t_idx(2:end)))/(8*T);
Case.mid.Pl = (Ppa + Pmid)/2;
Case.mid.Ptm = sum((Case.mid.Pl -   Case.mid.Pim).*Dt(t_idx(2:end)))/(8*T);
Case.mid.PC =   sum((Pmid -   Case.mid.Pim).*Dt(t_idx(2:end)))/(8*T);
Case.mid.Q = sum((Qmid).*Dt(t_idx(2:end)))/(8*T);
Case.mid.Pc = sum((Pmid).*Dt(t_idx(2:end)))/(8*T);
...
Case.epi.RA = sum(((Ppa - Pepi)./Qepi).*Dt(t_idx(2:end)))/(8*T);
Case.epi.Pl = (Ppa + Pepi)/2;
Case.epi.Ptm = sum((Case.epi.Pl -   Case.epi.Pim).*Dt(t_idx(2:end)))/(8*T);
Case.epi.PC =   sum((Pepi -   Case.epi.Pim).*Dt(t_idx(2:end)))/(8*T);
Case.epi.Q = sum((Qepi).*Dt(t_idx(2:end)))/(8*T);
Case.epi.Pc = sum((Pepi).*Dt(t_idx(2:end)))/(8*T);
...
Case.QPA = sum(QPA.*Dt(t_idx(2:end)))/(8*T);
